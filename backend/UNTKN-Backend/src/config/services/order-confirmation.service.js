...
export const sendOrderConfirmationEmail = async (orderId) => {
    ...
    let deliveryName = "STANDARD DELIVERY";
    let deliveryDescription = "5–7 BUSINESS DAYS";

    if (order.delivery_method) {
        const [deliveryMethods] = await pool.execute(
            `
            SELECT
                name,
                description
        ...
        if (deliveryMethods.length > 0) {
            deliveryName =
                deliveryMethods[0].name || deliveryName;

            deliveryDescription =
                deliveryMethods[0].description ||
                deliveryDescription;
        }
    }
...
Estimated delivery: ${deliveryDescription}
            ...
            <div style="padding:45px 40px;">
                ...
                <div style="margin-top:42px;padding-top:30px;border-top:1px solid #eeeeee;">
                    ...
                    <div style="font-size:14px;color:#333;line-height:1.8;">
                        <strong>${escapeHtml(deliveryName)}</strong><br>
                        ${escapeHtml(deliveryDescription)}
                    </div>
                </div>
